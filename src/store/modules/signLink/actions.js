...
export const editLinkStatus = ({ commit }, formData) => {
  SignLink.update(formData)
    .then((response) => {
      commit("SET_LINK_UPDATE", response.data.data);
      toast.success('Link successfully deactivated', {
        timeout: 5000,
        position: "top-right",
      });
    })
...
export const resourceTools = ({ commit, getters }, formData) => {
  ...
  SignLink.storeTools(formData)
    .then(() => {
      SignLink.allTools(getters.link.id)
        .then((response) => {
          commit("SET_TOOLS", response.data.data);
          commit("SET_LOADER", false)
        });
...
export const editTools = ({ commit }, formData) => {
  ...
  SignLink.updateTool(formData.id, formData.payload)
    .then((response) => {
      commit("SET_EDIT_TOOL", response.data.data);
      setTimeout(() => commit("SET_TOOL_LOADER", { id: '', active: false }), 1000);
    })
    .catch((error) => {
      console.log(error);
    });
